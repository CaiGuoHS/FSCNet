...
def main():
    parser = argparse.ArgumentParser(description='FSCNet')
...
def train(device, args):
    ...
    fscnet = model(num_resblocks=[7, 7, 7, 7], input_channels=[3, 6, 6, 6]).to(device)
    sfnet_optim = torch.optim.Adam(fscnet.parameters(), lr=args.learning_rate)
    ...
    if args.resume:
        ...
        fscnet.load_state_dict(torch.load(str('./ckpts/FSCNet.pth')))
        print('load fscnet success')
    ...
    for epoch in range(START_EPOCH + 1, args.epochs):
        ...
        for target_image, input_image in train_dataloader:
            ...
            output_image = fscnet(input_image)[0]
        ...
        torch.save(fscnet.state_dict(), str('./ckpts/FSCNet.pth'))
        ...
        if (epoch + 1) % 500 == 0:
            ...
            torch.save(fscnet.state_dict(), str('./ckpts/epoch' + str(epoch) + '/fscnet.pth'))
